...
const http = require('http').createServer(app);//소켓.io
...
MongoClient.connect(process.env.DB_URL, function (에러, client) {
  ...
  http.listen(process.env.PORT, function () { //기존 app.listen 소켓io 에서는 http.listen
    //8080 서버 열렸을때 함수실행
...
app.post('/message', 로그인했니, function(요청, 응답){
  ...
  app.get('/socket', function (요청, 응답) { //13분부터 듣기%%$%$#%
    응답.render('socket.ejs')
  })
  io.on('connection', function(socket){ //접속되면 함수 실행, 연결된 파일에 var socket = io(); 입력해줘야함
    console.log('연결되었어요');

    socket.on('joinroom', function (data) {//접속한 페이지에서 실행한 joinroom 이벤트 실행
      socket.join('room1') //채팅방 생성
    })
  

    socket.on('user-send', function(data){ //접속한 페이지에서 실행한 user-send 이벤트 실행
      console.log(data)
      io.emit('broadcast', data) //모든사람에게 데이터 전송
      //io.to(socket.id).emit('broadcast', data); //특정사람에게만 전달 여기서 socket.id는 자기의 id가 들어가므로 자기에게만 데이터가 전달됨
    });
